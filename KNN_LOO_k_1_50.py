...
def KNN_Leave_One_Out_Accuracy_Validator(k_neighbours, training_set, training_labels):
    successful_predictions  = 0
    for i in range(0,len(training_set)):
        prediction = KNN_Classifier(k_neighbours, training_set[:i]+training_set[i+1:], training_labels, training_set[i])
        if prediction == training_labels[i]:
            successful_predictions+=1
            #print("Hit!", hit)

    accuracy = successful_predictions/len(training_labels)
   # print("KNN_Classifier (number of nearest neighbours:",k_neighbours,"), Leave-One-Out Validation Method,Estimated accuracy :", round(accuracy, 3)) 
    return(round(accuracy, 4))   
#%%
import sys

saveout = sys.stdout
save = open("KNN_LOO_k_from_1_to_50_sans_i.txt", 'w')   #saving in a file the output of print
sys.stdout = save

accuracy_multiple_k = []
for k in range(1,50):
    accuracy = KNN_Leave_One_Out_Accuracy_Validator(k, training_set, training_labels)
    accuracy_multiple_k.append(k)
    print("KNN_LOO, k = "+str(k)+", accuracy:", accuracy)
